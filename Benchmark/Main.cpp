...
#define MAX_FOR_QUAD (std::thread::hardware_concurrency() * 2000)
...
double quadratic_formula() {
    ...
    for (unsigned int i = 1; i < MAX_FOR_QUAD; i++) {
...
bool askRunTest(std::string&& testName) { 
    while (1) {
        std::cout << "run benchmark: " << testName << "(y/N)?:";
        std::string response;
        std::cin >> response;
...
int main() {
    ...
    if (askRunTest("Quadratic Formula")) {
